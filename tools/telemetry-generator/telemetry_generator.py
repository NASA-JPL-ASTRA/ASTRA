...
class TelemetryLogger:
    ...
    def flush(self):
        ...
        with open(self.channel_log_path, "w", newline="") as f:
            writer = csv.writer(f)
            for timestamp, channel, value in rebased_channel_rows:
                writer.writerow([self._format_timestamp(timestamp), channel, value])

        # Write event log (no header)
        with open(self.event_log_path, "w", newline="") as f:
            writer = csv.writer(f)
            for timestamp, evr_name, severity, message in rebased_event_rows:
                writer.writerow([self._format_timestamp(timestamp), evr_name, severity, message])
    ...
    def _rebase_timestamps(self, rows: List[tuple], interval_seconds: int = 2) -> List[tuple]:
        ...
        for idx, row in enumerate(rows):
            offset_steps = total_rows - 1 - idx
            rebased_timestamp = aligned_now - (offset_steps * interval_seconds)
            rebased_rows.append((rebased_timestamp, *row[1:]))

        return rebased_rows

    def _format_timestamp(self, timestamp: float) -> str:
        """Format unix timestamp into local readable datetime string."""
