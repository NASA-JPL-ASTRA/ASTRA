...
def get_channel_value(session_id: str, channel: str, at_time: float) -> dict | None:
    ...
    Flux query explanation:
        range(start: -30d, stop: at_time)  — look at all data up to at_time
        filter session_id and channel      — scope to exactly what we want
        last()                             — take the most recent point in that window
    """
