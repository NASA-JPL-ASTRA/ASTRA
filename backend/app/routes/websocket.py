...
async def websocket_endpoint(websocket: WebSocket, sid: str):
    ...
    """
    # Accept first (required for handshake), then validate session
    await websocket.accept()

    session = get_session(sid)
    logger.info(f"WebSocket connected: sid={sid}, session_found={session is not None}")
    if not session:
        logger.warning(f"WebSocket rejected: session {sid} not found")
        await websocket.send_text(json.dumps({
            "event": "error.occurred",
            "session_id": sid,
            "data": {"message": f"Session {sid} not found"},
        }))
