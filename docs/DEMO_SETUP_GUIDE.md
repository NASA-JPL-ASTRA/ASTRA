...
Demo 由三个部分组成，建议目录结构如下：

```
your-workspace/
├── Fast-Powerful-Whisper-AI-Services-API/   # Whisper API + 启动脚本
└── ASTRA-dev-feature1/
...
```bash
# 克隆 Whisper API 项目
git clone https://github.com/YOUR_ORG/Fast-Powerful-Whisper-AI-Services-API.git
cd Fast-Powerful-Whisper-AI-Services-API

# 克隆 ASTRA 项目（与 Whisper 项目同级目录）
cd ..
git clone https://github.com/YOUR_ORG/ASTRA-dev-feature1.git
```
...
# 设置 Backend 路径（若不在默认位置）
export ASTRA_BACKEND_PATH="../ASTRA-dev-feature1/backend"

# 启动 Whisper API + Backend
...
```bash
cd ASTRA-dev-feature1/frontend
npm run dev
```
...
```bash
cd ASTRA-dev-feature1\backend
set WHISPER_API_URL=http://127.0.0.1:8001
...
```bash
# macOS/Linux
export ASTRA_BACKEND_PATH="/绝对路径/ASTRA-dev-feature1/backend"

# Windows (PowerShell)
$env:ASTRA_BACKEND_PATH = "C:\path\to\ASTRA-dev-feature1\backend"
```
...
| ASTRA Backend | `ASTRA-dev-feature1/backend/requirements.txt` |
| Frontend | `ASTRA-dev-feature1/frontend/package.json` |
