...
REM Backend path: prefer local backend, then env var, then legacy locations
if "%ASTRA_BACKEND_PATH%"=="" (
  if exist "backend" (
    set ASTRA_BACKEND_PATH=%CD%\backend
  ) else if exist "..\ASTRA-dev-feature1\backend" (
...
echo.
REM Prefer local frontend, then sibling of backend
if exist "frontend" (
  echo Or start Frontend (in another terminal):
  echo   cd frontend ^&^& npm run dev
) else if not "%ASTRA_BACKEND_PATH%"=="" (
